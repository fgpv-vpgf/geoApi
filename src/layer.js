...
* the original id value is copied in a newly created property origID of the properties object
* and the existing id value is replaced by an autogenerated number.
* Features without existing id from that same dataset will get a new properties origID
...
function assignIds(geoJson) {
    ...
    geoJson.features.forEach(function (val, idx) {
        let idBckp = { origID: '' };
        Object.assign(val.properties, idBckp);

        if ('id' in val && typeof val.id !== 'undefined') {
            val.properties.origID = val.id;
            empty = false;
        }
        val.id = idx + 1;
    });

    // remove idOri if all empty
    if (empty) {
        geoJson.features.forEach(function (val) {
            delete val.properties.origID;
        });
    }
}
