...
class AqlLike extends AqlDiatomic {
    ...
    evaluate (attribute) {
        ...
        // find the escapeVal inside '' if exists
        if (this.escapeVal) {
            const realEscapeVal = this.escapeVal.match(/'([^']+)'/);
            this.escapeVal = realEscapeVal ? realEscapeVal[1] : this.escapeVal;
        }
        ...
        // future-proof in that it should work for most other escape characters other than ௌ should it get changed in plugins
        if (this.escapeVal) {
